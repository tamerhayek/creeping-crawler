...
Il Gold Standard è costruito manualmente e salvato come file JSON in \texttt{gs\_data/} (uno per dominio), che fungono da seed. Il DBMS è \textbf{MariaDB}. Lo schema (definito in \texttt{init.sql} ed esposto da \texttt{GET /db\_schema}) è organizzato in tre tabelle (Tabella~\ref{tab:schema}): \texttt{web\_resources} conserva l'HTML grezzo, \texttt{gold\_standard} il testo di riferimento in relazione 1-a-1, ed \texttt{evaluations} le metriche e il \texttt{judge\_score}. Solo le prime due sono richieste dalla specifica; \texttt{evaluations} è una tabella aggiuntiva che abbiamo introdotto come archivio dei risultati.
...
\paragraph{Healthcheck e avvio ordinato.} MariaDB e Ollama espongono un healthcheck e diventano \emph{healthy} solo quando sono realmente pronti: MariaDB quando accetta connessioni, Ollama dopo il download del modello \texttt{llama3.2:3b}. Il backend si avvia solo a quel punto, grazie ai \texttt{depends\_on} con \texttt{condition:\ service\_healthy}, e il frontend parte a sua volta dopo il backend. L'avvio è quindi ordinato e il sistema è utilizzabile solo quando tutte le dipendenze lo sono, gestendo anche il primo boot in cui il pull del modello richiede alcuni minuti.
