"""Internal library for the Crawl4AI Evaluation API.

Subpackages:
    crawling/       — web crawling via Crawl4AI
    evaluation/     — tokenization and metrics
    gold_standard/  — GS data loading and URL management
    parsers/        — domain-specific content parsers

Routes should import exclusively from this package (e.g. `from ..lib import fetch_page`)
rather than from individual submodules, so internal restructuring does not affect routes.
"""

from .crawling.crawler import fetch_page, PageContent
from .evaluation.metrics import TokenMetrics, calculate_metrics
from .evaluation.tokens import extract_unique_tokens, strip_markdown
from .gold_standard.gold import get_entry_for_url, load_gold_text, load_gold_tokens
from .gold_standard.urls import (
    get_all_entries,
    get_available_urls,
    get_domains,
    get_urls_for_domain,
    is_supported_domain,
)
from .parsers import get_parser_for_url
from .services import assert_supported_domain, build_gold_entry, compute_token_eval, domain_of
