...
\paragraph{Note on \texttt{www.cnbc.com} — GET fallback.} CNBC pages produce particularly large HTML payloads (up to several MB). Since the \texttt{POST /parse} request body cannot exceed a size limit, the frontend automatically falls back to \texttt{GET /parse?url=} when the \texttt{html\_text} exceeds that threshold: in this case the page is re-fetched live rather than re-processed from the cached HTML. This behaviour is expected and intentional; during tests on the CNBC domain \textbf{warnings} will appear signalling the use of the GET fallback.
